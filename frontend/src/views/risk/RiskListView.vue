<template>
  <div class="risk-list">
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <el-select v-model="queryParams.status" placeholder="状态" clearable style="width: 120px" @change="handleSearch">
          ...
          <el-option label="已发生" value="realized" />
        </el-select>
        <el-select v-model="queryParams.riskLevel" placeholder="等级" clearable style="width: 100px" @change="handleSearch">
    ...
    <el-row :gutter="16" style="margin-bottom: 16px">
      <el-col :span="6" v-for="stat in riskStats" :key="stat.riskLevel">
        <el-card shadow="hover" class="risk-stat-card" :class="stat.riskLevel">
    ...
    <el-table v-loading="loading" :data="riskList" stripe>
      ...
      <el-table-column prop="riskLevel" label="等级" width="80">
        <template #default="{ row }">
          <el-tag :type="levelType(row.riskLevel)" size="small">{{ levelLabel(row.riskLevel) }}</el-tag>
        </template>
      </el-table-column>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="概率">
              <el-select v-model="formData.probability">
                <el-option label="高" value="high" />
                <el-option label="中" value="medium" />
                <el-option label="低" value="low" />
              </el-select>
          ...
          <el-col :span="12">
            <el-form-item label="影响">
              <el-select v-model="formData.impact">
                <el-option label="高" value="high" />
                <el-option label="中" value="medium" />
                <el-option label="低" value="low" />
              </el-select>
...
import { getRiskList, createRisk, updateRisk, deleteRisk, updateRiskStatus } from '@/api/risk'
...
const total = ref(0)
const queryParams = reactive({ page: 1, size: 10, keyword: '', status: '', riskLevel: '' })
...
const formData = reactive<RiskCreateRequest>({
  title: '', description: '', category: 'technical', probability: 'medium', impact: 'medium', mitigationPlan: '', contingencyPlan: '',
})
...
const statusLabelMap: Record<string, string> = { open: '待处理', monitoring: '监控中', mitigated: '已缓解', closed: '已关闭', realized: '已发生' }
const statusTypeMap: Record<string, 'info' | 'warning' | 'success' | 'danger'> = {
  open: 'danger', monitoring: 'warning', mitigated: 'success', closed: 'info', realized: 'danger',
}
...
const riskStats = computed(() => [
  { riskLevel: 'critical', label: '严重风险', count: riskList.value.filter((r) => r.riskLevel === 'critical' && r.status !== 'closed').length },
  { riskLevel: 'high', label: '高风险', count: riskList.value.filter((r) => r.riskLevel === 'high' && r.status !== 'closed').length },
  { riskLevel: 'medium', label: '中风险', count: riskList.value.filter((r) => r.riskLevel === 'medium' && r.status !== 'closed').length },
  { riskLevel: 'low', label: '低风险', count: riskList.value.filter((r) => r.riskLevel === 'low' && r.status !== 'closed').length },
])
...
function handleCreate() {
  ...
  Object.assign(formData, { title: '', description: '', category: 'technical', probability: 'medium', impact: 'medium', mitigationPlan: '', contingencyPlan: '' })
...
async function handleClose(row: Risk) {
  try {
    ...
    await updateRiskStatus(row.id, { status: 'closed' })
